...
for($yearDiff = 0; $yearDiff <=2; $yearDiff++) {
    ...
    for($month = 12; $month >= 1; $month--) {
        ...
        $periods[$date->format('Y-m')] = ucwords(utf8_encode(strftime('%B de %Y', $date->getTimestamp())));
    }
}
...
$balance = getTimeStringFromSecond(abs($sumOfWorkedTime - $expectedTime));
$sign = ($sumOfWorkedTime >= $expectedTime) ? '+' : '-';

loadTemplateView('monthly_report', [
    'report' => $report,
    'sumOfWorkedTime' => getTimeStringFromSecond($sumOfWorkedTime),
